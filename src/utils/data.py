...
def get_2d_data(type: Literal["gaussian", "checkerboard"], n_samples: int = 10000) -> TensorDataset:
    if type == "gaussian":
        ...
        dataset /= 1.414  # Normalize roughly to [-2, 2] -> [-1.5, 1.5] range mostly
        ...
        # Normalize to be roughly centered and scaled
        dataset = dataset / 2.0
        return TensorDataset(torch.from_numpy(dataset))
...
def get_data(type: Literal["2d", "CIFAR10", "MNIST"], 
             batch_size: int = 128, 
    ...
    if type == "2d":
        if subtype is None:
            raise ValueError("subtype must be specified for 2d data (gaussian or checkerboard)")
